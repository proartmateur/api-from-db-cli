...
Capturar datos de conexión o leerlos desde un archivo de configuración.
Conectarse a PostgreSQL o SQL Server.
Mostrar objetos disponibles en la base de datos.
Permitir seleccionar una tabla.
Leer columnas y tipos de datos.
Resolver la configuración de soft delete mediante deleted_at.
Generar un comando compatible con un generador externo de APIs.
Ejecutar el comando generado previa confirmación.
Mostrar el resultado de la generación.
3. Alcance funcional
...
Selección manual.
Creación de nuevo campo deleted_at.
...
Generar el SQL necesario para alterar la tabla.
Mostrar el SQL antes de ejecutarlo.
Permitir copiar el SQL.
Permitir cancelar la operación.
Solicitar confirmación explícita antes de ejecutar.
Ejecutar el SQL solo si el usuario confirma.
Refrescar la estructura de la tabla después de ejecutar.
Verificar que el campo deleted_at existe.
Continuar con la generación solo si el campo fue verificado.
...
El sistema deberá copiar el SQL al portapapeles si el entorno lo permite.
Si no es posible copiarlo, deberá mostrarlo en pantalla.
El sistema deberá permitir que el usuario lo ejecute manualmente.
Después, el usuario podrá solicitar refrescar la estructura de la tabla.
El sistema deberá verificar si deleted_at ya existe.
RN-014: Cancelación de ALTER TABLE

Si el usuario cancela la operación:

No se ejecutará ningún SQL.
La base de datos permanecerá intacta.
El sistema deberá preguntar si desea continuar sin endpoint delete.
El sistema deberá permitir regresar a la selección de campo para soft delete.
...
Usar un tipo por defecto configurable.
Advertir al usuario.
Permitir continuar si el usuario acepta.
...
Permita ingresar conexión manual.
Permita leer conexión desde archivo.
Permita conectar a PostgreSQL.
Permita conectar a SQL Server.
Liste schemas y tablas.
Liste funciones en PostgreSQL.
Liste stored procedures en SQL Server.
Permita seleccionar una tabla.
Lea columnas y tipos de datos.
Normalice tipos para el generador.
Detecte deleted_at automáticamente.
Permita seleccionar manualmente un campo para soft delete.
Pregunte si se desea endpoint delete.
Genere SQL para crear deleted_at si se requiere.
Muestre el SQL antes de ejecutarlo.
Permita copiar el SQL.
Permita cancelar sin modificar la base de datos.
Ejecute ALTER TABLE solo con confirmación explícita.
Refresque metadata después de alterar la tabla.
Verifique que deleted_at existe.
Genere el comando para gen.exe.
Muestre el comando antes de ejecutarlo.
Permita copiar o cancelar el comando.
Ejecute el comando solo con confirmación.
Espere a que el comando termine.
Muestre stdout, stderr y código de salida.
No exponga contraseñas.
Maneje errores sin cerrar inesperadamente la aplicación.
...
Diferencias entre tipos de datos PostgreSQL y SQL Server.
Permisos insuficientes para leer metadata.
Permisos insuficientes para ejecutar ALTER TABLE.
Tablas sin primary key.
Tablas con nombres reservados.
Tablas o columnas con espacios o caracteres especiales.
Tipos de datos no soportados por el generador.
Stored procedures con parámetros complejos.
Funciones PostgreSQL con firmas sobrecargadas.
Diferencias entre sistemas operativos al ejecutar gen.exe.
Riesgo de modificar estructura en una base productiva.
Ambigüedad futura sobre generación desde funciones o procedures.
...
Generación segura de SQL ALTER TABLE.
Vista previa y confirmación de SQL.
Generación de comando.
Ejecución de comando externo.
Media prioridad
Listado de funciones PostgreSQL.
Listado de stored procedures SQL Server.
Selección manual de campo deleted_at.
Copia de SQL al portapapeles.
Copia de comando al portapapeles.
Logs de ejecución.
Archivo de configuración.
Baja prioridad inicial
Generación desde stored procedures.
Generación desde funciones.
Persistencia segura de múltiples conexiones.
...
Formato exacto del archivo de configuración:
JSON.
TOML.
YAML.
ENV.
Formato exacto esperado por gen.exe para:
Nombre de tabla.
Nombre de schema.
Tipos de datos.
Activación de endpoint delete.
Campo deleted_at.
Si el comando debe recibir schema:
gen.exe public.users id:int,name:string

o solo tabla:

gen.exe users id:int,name:string
Si el endpoint delete se activa implícitamente por presencia de deleted_at o mediante argumento explícito.
Si el SQL de PostgreSQL debe usar TIMESTAMP o TIMESTAMPTZ.
Si se soportará selección múltiple de tablas en la primera versión.
Si las credenciales se guardarán localmente.
Si la primera interfaz será:
CLI.
TUI con Ratatui.
GUI con Tauri.
...
Tenga implementación funcional.
Tenga manejo básico de errores.
No exponga credenciales.
Tenga pruebas unitarias cuando aplique.
Tenga al menos una prueba manual documentada.
Respete las reglas de confirmación para operaciones sensibles.
Sea independiente de la UI cuando pertenezca al core.
Devuelva mensajes claros al usuario.
